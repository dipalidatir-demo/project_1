...
const createAuthor = async function (req, res) {
    ...
    try {
      ...
      if (!Object.keys(data).length)
        return res
          .status(400)
          .send({ status: false, msg: "Please Provides the Author Details" });
      ...
      if (!isValid(data.lname))
        return res
          .status(400)
          .send({ status: false, msg: "LastName is Required" });
      ...
      if (emailCheck)
        return res
          .status(400)
          .send({ status: false, msg: "Email-Id already Registerd" });
  
      let saveData = await authorModel.create(data);
  
      res.status(201).send({
        status: true,
        msg: "Author Created Sucessfully",
        data: saveData,
      });
    } catch (err) {
  ...
  const login = async function (req, res) {
    try {
      ...
      if (!Object.keys(data).length)
        return res.status(400).send({
          status: false,
          msg: "Please Provide the Correct Login Details",
        });
      ...
      if (!user)
        return res
          .status(401)
          .send({ status: false, msg: "EmailId or Password incorrect" });
