...
const createBlog = async function (req, res) {
  try {
    ...
    if (!Object.keys(data).length)
      return res
        .status(400)
        .send({ status: false, msg: "Please Provide Valid Blog Details" });
    ...
    if (checkBlog)
      return res
        .status(400)
        .send({ status: false, msg: "Blog already exists" });
...
const getSpecificAllBlogs = async function (req, res) {
    try {
      ...
      let blogData = await blogModel
        .find({ $and: [data, { isDeleted: false }, { isPublished: true }] })
        .populate("authorId");
  ...
  const deleteBlog = async function (req, res) {
    try {
      ...
      if (deletedBlog.modifiedCount == 0)
        return res
          .status(400)
          .send({ status: false, msg: "No Blog Document Exists" });
